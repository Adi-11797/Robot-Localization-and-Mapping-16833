...
def create_linear_system(odoms, observations, sigma_odom, sigma_observation,
    ...
    for r in range(n_obs):
        ...
        b_obsrv = np.transpose(observations[r,2:4])

        # Solving for matrices A and b (landmark)

        #jacobian_lm = [[-1,0,1,0],[0,-1,0,1]]
        #A[2*(n_odom + 1 + r):2*(n_odom + 1 + r + 1), (2*(n_poses + j)):(2*(n_poses + j + 1))] = sqrt_inv_obs@jacobian_lm   #-------------checkerror!!!!!
        
        A[2*(n_odom + 1 + r):2*(n_odom + 1 + r + 1), 2*i:2*(i+1)] = -(sqrt_inv_obs @ np.eye(2))
        
        A[(2*(n_odom + 1 + r)):(2*(n_odom + 1 + r + 1)), (2*(n_poses + j)):(2*(n_poses + j + 1))] = sqrt_inv_obs @ np.eye(2)

        
        b[(2*(n_odom + 1 + r)):(2*(n_odom + 1 + r + 1))] = sqrt_inv_obs @ b_obsrv
