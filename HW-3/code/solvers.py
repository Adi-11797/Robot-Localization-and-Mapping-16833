...
def solve_lu(A, b):
    ...
    slu_solver_natural = splu(csc_matrix(A.T@A),permc_spec='NATURAL')
    U = slu_solver_natural.U
    x = slu_solver_natural.solve(A.T@b)
    return x, U
...
def solve_lu_colamd(A, b):
    ...
    slu_solver_colamd = splu(csc_matrix(A.T @ A), permc_spec='COLAMD')
	U = slu_solver_colamd.U
    x = slu_solver_colamd.solve(A.T @ b)
    return x, U
