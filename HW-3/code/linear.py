...
def create_linear_system(odoms, observations, sigma_odom, sigma_observation,
    ...
    b[0:2]=np.zeros((2,))
    A[0:2,0:2] =  np.array([[sqrt_inv_odom, 0.0],
                            [0.0, sqrt_inv_odom]])
    ...
    for i in range(n_odom):
        ...
        A[2*(i+1):2*(i+2), 2*i:2*(i+1)] = np.array([[-sqrt_inv_odom, 0.0],
                                                    [0.0, -sqrt_inv_odom]])
        
        A[2*(i+1):2*(i+2), 2*(i+1):2*(i+2)] = np.array([[sqrt_inv_odom, 0.0],
                                                        [0.0, sqrt_inv_odom]])
    ...
    for r in range(n_obs):
        ...
        b_obs = np.transpose(observations[r,2:4])
        
        b[(2*(n_odom + 1 + r)):(2*(n_odom + 1 + r + 1))] = sqrt_inv_obs @ b_obs
        
        A[2*(n_odom + 1 + r):2*(n_odom + 1 + r + 1), 2*i:2*(i+1)] = np.array([[-sqrt_inv_obs, 0.0],
                                                                              [0.0, -sqrt_inv_obs]])
        
        A[(2*(n_odom + 1 + r)):(2*(n_odom + 1 + r + 1)), (2*(n_poses + j)):(2*(n_poses + j + 1))] = np.array([[sqrt_inv_obs, 0.0],
                                                                                                              [0.0, sqrt_inv_obs]])
